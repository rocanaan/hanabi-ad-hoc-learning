...
AGENT_CLASSES = {'RainbowAgent1':None,'RainbowAgent2':None,'RainbowAgent3':None,'RainbowAgent4':None,'RainbowAgent5':None}
...
flags.DEFINE_string('checkpoint_dir2', '',
                    'Directory where checkpoint files should be saved. If '
                    'empty, no checkpoints will be saved.')
flags.DEFINE_string('checkpoint_dir3', '',
                    'Directory where checkpoint files should be saved. If '
                    'empty, no checkpoints will be saved.')
flags.DEFINE_string('checkpoint_dir4', '',
                    'Directory where checkpoint files should be saved. If '
                    'empty, no checkpoints will be saved.')
flags.DEFINE_string('checkpoint_dir5', '',
                    'Directory where checkpoint files should be saved. If '
                    'empty, no checkpoints will be saved.')
...
def launch_experiment(agentX,environment,obs_stacker,start_iteration, experiment_checkpointer,my_agent,num_agent):
  ...
  if num_agent == '1':
    checkpoint_dir2 = FLAGS.checkpoint_dir1
  elif num_agent == '2':
    checkpoint_dir2 = FLAGS.checkpoint_dir2
  elif num_agent == '3':
    checkpoint_dir2 = FLAGS.checkpoint_dir3
  elif num_agent == '4':
    checkpoint_dir2 = FLAGS.checkpoint_dir4
  elif num_agent == '5':
    checkpoint_dir2 = FLAGS.checkpoint_dir5
  ...
  if agentX[:12] == 'RainbowAgent':
    latest_checkpoint_version = checkpointer.get_latest_checkpoint_number(checkpoint_dir2)
    start_iteration2, experiment_checkpointer2 = (
...
def main(unused_argv):
  ...
  for agent_2 in agent_list:
    if (agent_2[:12] != 'RainbowAgent'):
      return_collection = launch_experiment(agent_2,environment,obs_stacker,start_iteration, experiment_checkpointer,my_agent,agent_2[12])
      temp_score_list.append(statistics.mean(return_collection))
      temp_std_list.append(statistics.stdev(return_collection))
    else:
      print(agent_2)
