...
COLOR_DICT = colors = {'R':0, 'Y':1, 'G':2, 'W':3, 'B':4}
...
# Note: currently only works for 2 players
def action_to_int(action):
  if action['action_type'].lower() == 'discard':
    base = 0
    offset = action['card_index']
  if action['action_type'].lower() == 'play':
    base = 5
    offset = action['card_index']
  if action['action_type'].lower() == 'reveal_color':
    base = 10
    offset = COLOR_DICT[action['color']]
  if action['action_type'].lower() == 'reveal_rank':
    base = 15
    offset = action['rank']
  return base + offset
...
def main(unused_argv):
  ...
  while True:
    ...
    for s in state_dir:
      if "state" in s:
        ...
        action = a1.act(data)
        print(action)
        output = {'action':action_to_int(action)}
        print(output)
  ...
  #### ORIGINAL CODE FROM BEHAVIORAL BELOW
  # my_agents_file = FLAGS.my_agents
